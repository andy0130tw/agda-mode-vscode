...
function getAgdaPaths() {
  if (inTestingMode.contents) {
    console.log("[XXXXXXXXXXXXXX] getAgdaPaths", agdaPathsInTestingMode.contents);
    return agdaPathsInTestingMode.contents;
  }
  var ret = parseAgdaPaths(Core__Option.getOr(Vscode.workspace.getConfiguration("agdaMode", undefined).get("connection.paths"), null));
  console.log("[XXXXXXXXXXXXXX] getAgdaPaths", ret);
  return ret;
}

function addAgdaPath(logChannel, path) {
  var paths = getAgdaPaths();
  console.log("[XXXXXXXXXXXXXX] addAgdaPath", paths, path);
...
function setAgdaPaths(logChannel, paths) {
  console.log("[XXXXXXXXXXXXXX] setAgdaPaths", paths);
  if (inTestingMode.contents) {
    Chan$AgdaModeVscode.emit(logChannel, {
          TAG: "Config",
          _0: {
