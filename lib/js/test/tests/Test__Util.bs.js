...
async function warmUpAgdaModeModule() {
  this.timeout(OS$AgdaModeVscode.onUnix ? 4000 : 10000);
  var ctx = await makeAndLoad("WarmUp.agda");
  var xxx = ctx.state.connection;
  if (xxx !== undefined) {
    console.log("[AGDA PATH IS]", Connection$AgdaModeVscode.getPath(xxx));
  } else {
    Assert.fail("No connection path found");
  }
...
exports.warmUpAgdaModeModule = warmUpAgdaModeModule;
