...
async function search(name, timeoutOpt) {
  ...
  if (stdout.TAG === "Ok") {
    var stdout$1 = stdout._0;
    console.log("[XXXXXXXXXXXXXXXXXX] OK", stdout$1);
    return {
            TAG: "Ok",
            _0: stdout$1
          };
  }
  var ret = await searchWith("where.exe", name, timeout);
  console.log("[XXXXXXXXXXXXXXXXXXXXXX] where?", ret);
  return ret;
}
